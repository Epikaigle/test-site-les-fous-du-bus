    ...
    >
      ...
      >
        <div className="flex items-center px-4 py-4 border-b border-white/10 bg-surface">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="text-textSecondary hover:text-textMain px-2 text-xs font-semibold tracking-wider"
          >
            ECHAP
          </button>
        </div>
            ...
            <ul className="py-2">
              {results.map((result, idx) => (
                <li key={idx}>
                  <a
                    href={result.url}
                    className="block px-6 py-4 hover:bg-white/5 border-b border-white/5 last:border-0 transition-colors"
                  >
                    <h4 className="text-textMain font-medium mb-1 text-[15px]">
                      {result.meta.title || 'Page'}
                    </h4>
                    <p
                      className="text-[13px] text-textSecondary line-clamp-2"
